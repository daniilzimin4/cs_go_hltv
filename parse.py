...
def get_match_info(match_id):
    ...
    match_info["team1"]["players"] = [player.text for player in
                                      page.find_all("table", {"class": "stats-table"})[0].find_all("td", {
                                          "class": "st-player"})]
    match_info["team2"]["players"] = [player.text for player in
                                      page.find_all("table", {"class": "stats-table"})[1].find_all("td", {
                                          "class": "st-player"})]
...
def _get_historical_lineup(player_anchors):
    ...
    for player_anchor in player_anchors[5::]:
        ...
        player['name'] = buildName[0].rstrip() + buildName[2]
        player['nickname'] = player_anchor.find("div", {"class": "teammate-info standard-box"}).find("div", {
            "class": "text-ellipsis"}).text.encode('utf8')
        player['maps-played'] = int(re.search(r'\d+',
                                              player_anchor.find("div", {"class": "teammate-info standard-box"}).find(
                                                  "span").text).group())
        players.append(player)
...
def get_upcoming_matches():  # updated
...
def get_live_matches():  # new fun
    matches = get_parsed_page("http://www.hltv.org/matches/")
    matches_list = []
    upcomingmatches = matches.find("div", {"class": "liveMatchesSection"})

    matchdays = matches.find_all("div", {"class": "liveMatchesSection"})

    for match in matchdays:
        matchDetails = match.find_all("div", {"class": "liveMatch"})
        for getMatch in matchDetails:
            matchObj = {}
